		...
		opts.customCommands.forEach(def => {
			...
			if (Array.isArray(def.options)) {
				def.options.forEach(opt =>
					cmd.option(opt.option, opt.description, opt.autocomplete)
				);
			}

			if (def.parse) cmd.parse(def.parse);

			if (def.types) cmd.types(def.types);

			if (def.help) cmd.help(def.help);

			if (def.validate) cmd.validate(def.validate);

			if (def.allowUnknownOptions) cmd.allowUnknownOptions();
			...
			if (def.cancel) cmd.cancel(def.cancel);
		});
	}

	// Start REPL
	vorpal.delimiter(opts.delimiter).show();
}
